...
public final class S2ValidatorFactory {
    ...
    /**
     * Forcefully registers (or replaces) a validator for the given context key.
     * <p>
     * Unlike {@link #getOrRegister(String, Supplier)}, this method always overwrites
     * any previously cached validator for the same key. Use this when validation rules
     * need to be updated at runtime (e.g., admin vs. user rules with the same form key).
     * </p>
     *
     * <p>
     * <b>[한국어 설명]</b>
     * </p>
     * 지정된 컨텍스트 키에 대해 검증기를 강제로 등록(또는 교체)합니다.
     * <p>
     * {@link #getOrRegister(String, Supplier)}와 달리 동일한 키에 이미 캐시된 검증기가
     * 있더라도 항상 덮어씁니다. 같은 폼 키로 관리자용/사용자용 규칙을 동적으로 전환해야 하는
     * 경우에 유용합니다.
     * </p>
     *
     * @param <T>       The target type handled by the validator | 검증기가 처리하는 대상 타입
     * @param contextKey Unique identifier for the validation context | 검증 컨텍스트의 고유 식별자
     * @param validator  The validator instance to register | 등록할 검증기 인스턴스
     * @return The registered validator instance | 등록된 검증기 인스턴스
     */
    public static <T> S2Validator<T> register(String contextKey, S2Validator<T> validator) {
        validatorCache.put(contextKey, validator);
        return validator;
    }

    /**
     * Removes a cached validator for the given context key.
     * <p>
     * After removal, the next call to {@link #getOrRegister(String, Supplier)} with the
     * same key will execute the supplier to create a fresh validator.
     * </p>
     *
     * <p>
     * <b>[한국어 설명]</b>
     * </p>
     * 지정된 컨텍스트 키에 대해 캐시된 검증기를 제거합니다.
     * <p>
     * 제거 후 동일한 키로 {@link #getOrRegister(String, Supplier)}를 호출하면
     * 서플라이어가 다시 실행되어 새로운 검증기가 생성됩니다.
     * </p>
     *
     * @param contextKey The context key to remove | 제거할 컨텍스트 키
     * @return The previously cached validator (or {@code null} if not found) | 이전에 캐시된 검증기 (없으면 null)
     */
    public static S2Validator<?> remove(String contextKey) {
        return validatorCache.remove(contextKey);
    }

    /**
     * Clears all cached validators.
     * <p>
     * This method is particularly useful for test isolation to prevent validation rules
     * from leaking across test cases. It is also useful when the application context
     * is restarted and all cached validators need to be rebuilt.
     * </p>
     *
     * <p>
     * <b>[한국어 설명]</b>
     ...
     * 모든 캐시된 검증기를 제거합니다.
     * <p>
     * 단위 테스트 간 상태 격리(Test Isolation) 시 검증 규칙이 다른 테스트로 오염(Leak)되는 것을
     * 방지하기 위해 유용합니다. 또한 애플리케이션 컨텍스트가 재시작되어 모든 캐시된 검증기를
     * 다시 빌드해야 하는 경우에도 사용할 수 있습니다.
     * </p>
     */
